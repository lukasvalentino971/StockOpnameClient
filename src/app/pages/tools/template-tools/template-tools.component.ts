...
@Component({
  selector: 'app-template-tools',
  templateUrl: './template-tools.component.html',
  styleUrls: ['./template-tools.component.css']
})
export class TemplateToolsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.MasterService.showTools(this.params).subscribe((res) => {
      this.data = res;
      this.url = `https://myapps.aio.co.id/stock-opname/tools/form/${this.data.id}`
      // this.url = `http://192.168.144.81:4200/tools/form/${this.data.id}`
