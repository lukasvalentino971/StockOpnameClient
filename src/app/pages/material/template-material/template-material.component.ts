...
@Component({
  selector: 'app-template-material',
  templateUrl: './template-material.component.html',
  styleUrls: ['./template-material.component.css']
})
export class TemplateMaterialComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.MasterService.showMaterial(this.params).subscribe((res) => {
      this.data = res;
      this.url = `https://myapps.aio.co.id/stock-opname/material/form/${this.data.id}`
      // this.url = `http://192.168.144.81:4200/material/form/${this.data.id}`
  ...
  exportPdf() {
    html2canvas(document.getElementById('table')!).then(canvas => {
      ...
      const width = pdf.internal.pageSize.getWidth() - 0.5;
      // const height = canvas.height ;
      // const height = canvas.height * width / canvas.width;
      const height = pdf.internal.pageSize.getHeight() - 0.5;
