...
@Component({
  selector: 'app-template-packaging',
  templateUrl: './template-packaging.component.html',
  styleUrls: ['./template-packaging.component.css']
})
export class TemplatePackagingComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.MasterService.showPackaging(this.params).subscribe((res) => {
      this.data = res;
      this.url = `https://myapps.aio.co.id/stock-opname/packaging/form/${this.data.id}`
      // this.url = `http://192.168.144.81:4200/packaging/form/${this.data.id}`
